...
- When uncertain, treat preconditions as unmet and effects as unachieved.
...
- CROSS-STEP INFORMATION RETENTION (IMPORTANT):
  ...
  - Whenever "Tool returned" contains data that can satisfy ANY precondition/effect (especially those mentioning things like "ID is known", "URL is known", "handle is available", "mapping between X and Y is known"):
    - In the "preconditions" array, set `"met": true` for the corresponding item and fill `"actual_title"` when appropriate.
    - In the "effects" array, set `"achieved": true` for the corresponding item and add supporting identifiers/values into `"values"` (e.g. IDs, URLs).
    ...
    - Add corresponding structured objects to "extracted_results" so that later steps can easily reuse these identifiers.
...
{{
  ...
  "extracted_results": [
    {{ "name": "Key attribute 1", "value": "...", "id": "optional-id-or-null", "url": "optional-url-or-null" }},
    {{ "name": "Key attribute 2", "value": "...", "id": "optional-id-or-null", "url": "optional-url-or-null" }}
  ],
  "preconditions": [
    {{
